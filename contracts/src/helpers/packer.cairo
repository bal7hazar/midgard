...
use midgard::helpers::math::fast_power;
...
> of PackerTrait<T, U, V> {
    fn get(packed: T, index: u8, size: V) -> U {
        let mask: T = fast_power(2_u8.into(), size.into()) - 1_u8.into();
        let offset: T = fast_power(2_u8.into(), index.into() * size.into());
    ...
    fn contains(mut packed: T, value: U, size: V) -> bool {
        let modulo: T = fast_power(2_u8.into(), size.into());
        let mut index = 0;
        loop {
            if packed.is_zero() {
                break false;
    ...
    fn replace(mut packed: T, index: u8, size: V, value: U) -> T {
        // [Compute] Mask
        let mut mask: T = fast_power(2_u8.into(), size.into()) - 1_u8.into();
        let offset: T = fast_power(2_u8.into(), index.into() * size.into());
        mask = mask * offset;
    ...
    fn pack(mut unpacked: Array<U>, size: V) -> T {
        ...
        let mut modulo: T = fast_power(2_u8.into(), size.into());
